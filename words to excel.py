...
# --- Step 1: Paste the full text from your DOCX file here ---
# Make sure the text retains the two-line structure you described.
document_text = """
...
# --- Configuration ---
output_filename = "extracted_questions.txt" # The name of the text file to create
# --- End Configuration ---


# Regular expression to capture the fields separated by '|' on a single line
regex = re.compile(
    r"^(?:\[source: \d+\]\s*)?(?:\s*)?"
    r"([^|]+?)\s*\|"
    r"([^|]+?)\s*\|"
...
extracted_data = []
header = ["Section/Rule", "Context/Area", "Question", "Responsible Party/Subject", "T-Level"]
...
for match in regex.finditer(document_text):
    ...
    responsible_party = match.group(4).strip()
    t_level = match.group(5).strip()

    if question.lower().startswith("will ") and question.endswith("?"):
         extracted_data.append([section, context, question, responsible_party, t_level])

# --- Write to File ---
try:
    # Get the directory where the script is running (important when run locally)
    ...
    print(f"\nSuccessfully parsed {len(extracted_data) - 1} questions.")
    # Provide confirmation and instructions assuming local execution
    print(f"Data successfully written to: {file_path}")
    print("You can now open this file with Notepad or import it into Excel/Sheets.")

except Exception as e:
    # Provide feedback if file writing fails (permission errors locally)
